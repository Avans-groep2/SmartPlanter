<template>
  <div class="linechart">
    ...
    <p>
      Laatste waarde:
      <span>{{ latestValue !== null ? latestValue + ' ' + unit : '-' }}</span>
    </p>
...
const props = defineProps({
  deviceId: { type: String, required: true, default: 'selectedDeviceId'},
...
async function loadData(deviceId) {
  ...
  const values = sorted.map(item => item.data[props.dataKey])
